...
### 项目"探索 + 理解 + 修改"任务行为约束
0. 当获取到项目名称关键词时, 先使用file_search工具对项目路径进行搜索
1. 当 file_search 返回多个候选路径时，你必须把这些候选视为一个待探索队列，而不是只选择第一个候选。
2. 每次只能选择一个候选路径进行验证，并优先使用 directory_list 查看其目录结构。
3. 对当前选中候选路径进行判断是否是满足用户所述项目特征, 若不是, 则判定无效. 例如, 如果某个候选目录为空，或它只包含一个子目录且该子目录为空，则该候选路径应判定为无效候选。
4. 一旦某个候选路径被判定为无效，不要继续在该候选路径或其子路径中执行 file_search、directory_list、file_read 或 file_write。
5. 当前候选无效时，必须回到最初的候选队列，选择下一个尚未探索的候选路径继续验证。
6. 只有当所有候选路径都验证失败后，才能得出“未找到有效项目目录”或“项目文件缺失”的结论。
7. 如果候选路径中同时存在父目录和子目录，例如 `flybird/` 与 `flybird/flybird_game/`，当子目录为空时，应重新评估父目录是否仍然包含有效项目文件；如果父目录没有其他有效文件，不要继续在该父目录中搜索。
